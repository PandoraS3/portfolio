...
export default function Contact() {
  ...
  const sendEmail = (e) => {
    ...
    emailjs.sendForm('service_lom7uwb', 'template_t9uk8qp ', form.current, 'ZfnfHPIxbwb7tfxHF')
      .then(() => {
  ...
  return (
    <section id="contact" style={{ padding: "120px 60px", background: "#050810" }}>
      <div style={{ maxWidth: "1300px", margin: "0 auto" }}>
        <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 1.2 }} viewport={{ once: true }} style={{ marginBottom: "80px" }}>
          <h2 style={{ fontSize: "4rem", color: "#fff", fontWeight: "900", marginBottom: "15px", letterSpacing: "-1px" }}>Kontakt</h2>
          <p style={{ color: "#7a8193", fontSize: "1.4rem", fontWeight: "300" }}>Bereit für das nächste Projekt? Ich freue mich sur Ihre Nachricht.</p>
        </motion.div>

        <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: "40px", alignItems: "stretch" }}>
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} viewport={{ once: true }} style={cardStyle}>
            <form ref={form} onSubmit={sendEmail} style={{ display: "flex", flexDirection: "column", gap: "25px" }}>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end" }}>
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} type="submit" style={btnSubmitStyle}>Nachricht senden</motion.button>
              </div>
            </form>
          </motion.div>

          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.4 }} viewport={{ once: true }} style={cardStyle}>
            ...
            <div style={{ display: "flex", gap: "15px", marginTop: "40px" }}>
              <motion.a href="https://github.com/PandoraS3" target="_blank" whileHover={{ y: -5 }} style={socialBtnStyle}>GitHub ↗</motion.a>
              <motion.a href="https://www.linkedin.com/in/millena-nyami-ngaha-887822229" target="_blank" whileHover={{ y: -5 }} style={socialBtnStyle}>LinkedIn ↗</motion.a>
            </div>
          </motion.div>
        </div>
      </div>
...
const btnSubmitStyle = { background: "#b4ff00", color: "#000", border: "none", padding: "18px 45px", borderRadius: "50px", fontWeight: "800", fontSize: "1.1rem", cursor: "pointer" };
const socialBtnStyle = { flex: 1, textAlign: "center", textDecoration: "none", color: "#b4ff00", border: "1px solid #b4ff00", padding: "15px", borderRadius: "50px", fontWeight: "700" };